---
tags:
  - theories
aliases:
  - Painting
---
![Paintings.excalidraw](Attachments/Paintings.excalidraw.md)
![](Attachments/Paintings.excalidraw.png)

- A Streamer mentioned that there’s lots of math in this game, such as tie time or lock clock?
- So far, they seem to be words missing a letter.
- Are they consistent between days?
	- Answer: seems like no. The order of these paintings probably matter?
- They're consistent in room positions. Closet and hallway are in the same location for days 2 and 3. Same with Den and Commissary.
- Maybe it works like a maze? Solution is to find the correct order of portraits and rooms?

![](Attachments/Pasted%20image%2020260324181558.png)
- As shown in the [Study](../Rooms/Study.md).
- The unused pairing letters: G, L, D, R, N
### Painting Locations

- In the [Parlor Room](../Rooms/Parlor%20Room.md): Dessert and Desert
- In the [[Closet]]: Bridge and Bride	
- Day 3: This was in the [Hallway](../Rooms/Hallway.md). 
- In the [Commissary](../Rooms/Commissary.md): Pine and Pin
	-  Day 3: This was in the [Den](../Rooms/Den.md).
- In the [Den](../Rooms/Den.md): Bat and Bath
- In the [Drawing Room](../Rooms/Drawing%20Room.md): Clock and Lock
- In the [Nook](../Rooms/Nook.md): Road and Rod
- In the [Pantry](../Rooms/Pantry.md): Stage and Sage
- In the [Bedroom & Guest Bedroom](../Rooms/Bedroom%20&%20Guest%20Bedroom.md): Pea and Peak
- In the [East Wing Hall](../Rooms/West%20Wing%20Hall%20&%20East%20Wing%20Hall.md): Car and Cart
- In the [[Bunk Bed]]: Stag and Tag

In the entrance hall there seems to be these 2 paintings as well. What they are, I can't tell.

![](Attachments/Pasted%20image%2020260323204700.png)
### Notes

- Does [Spare Room](../Rooms/Spare%20Room.md) not have paintings?
- Does [[Passageway]] not have paintings?
- Does [Chapel](../Rooms/Chapel.md) not have paintings (or all red rooms I guess)?
- Do #rooms/green-room not have paintings?

- Since Ten and Tag share the same painting, we should assume that the painting may not mean the same word every time.

- Hmm, the missing letter of row two spells "EIGHT"
- The missing letters probably spell out a message.